...
USE SCHEMA SE_FEATURES.CORTEX_FEAT;
USE WAREHOUSE SC_ASSISTANT_WH;
USE ROLE SYSADMIN;

-- Scale up warehouse for PDF parsing (compute-intensive operation)
ALTER WAREHOUSE SC_ASSISTANT_WH SET WAREHOUSE_SIZE = 'X-LARGE';
...
SELECT RELATIVE_PATH, SNOWFLAKE.CORTEX.PARSE_DOCUMENT(@SUPPLY_CHAIN_ASSISTANT_PDF_STAGE,RELATIVE_PATH,{'mode':'LAYOUT'}) AS DATA
    FROM DIRECTORY(@SUPPLY_CHAIN_ASSISTANT_PDF_STAGE);
...
CREATE OR REPLACE TABLE PARSED_PDFS AS (
    ...
    SELECT
        ...
        'SE_FEATURES.CORTEX_FEAT.SUPPLY_CHAIN_ASSISTANT_PDF_STAGE' AS INPUT_STAGE,
        RELATIVE_PATH AS RELATIVE_PATH,                                    -- Original file path
        GET_PRESIGNED_URL(@SE_FEATURES.CORTEX_FEAT.SUPPLY_CHAIN_ASSISTANT_PDF_STAGE, RELATIVE_PATH, 604800) AS PRESIGNED_URL  -- Clickable URL (expires in 7 days)
...
ALTER WAREHOUSE SC_ASSISTANT_WH SET WAREHOUSE_SIZE = 'SMALL';
...
CREATE OR REPLACE CORTEX SEARCH SERVICE SUPPLY_CHAIN_INFO
ON PAGE_CONTENT                                      -- Primary search field
WAREHOUSE = SC_ASSISTANT_WH
...
CREATE OR REPLACE TASK REFRESH_PRESIGNED_URLS_TASK
  WAREHOUSE = SC_ASSISTANT_WH
  SCHEDULE = 'USING CRON 0 1 * * 0 America/Los_Angeles'  -- Runs weekly on Sunday at 1:00 AM PST
AS
UPDATE PARSED_PDFS
SET PRESIGNED_URL = GET_PRESIGNED_URL(@SUPPLY_CHAIN_ASSISTANT_PDF_STAGE, RELATIVE_PATH, 604800);

-- Activate the task to start automatic URL refresh
ALTER TASK REFRESH_PRESIGNED_URLS_TASK RESUME;

EXECUTE TASK REFRESH_PRESIGNED_URLS_TASK;